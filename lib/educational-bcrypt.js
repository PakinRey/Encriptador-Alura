(function (root, factory) {
  ...
  // Hash educativo simplificado (NO criptográfico): sirve para demostrar rounds y salting.
  function simpleHash(input) {
    // FNV-1a offset basis (32-bit), usado como semilla inicial.
    let h1 = 0x811c9dc5;
    // Constante de mezcla secundaria (golden ratio fraccional en 32-bit).
    let h2 = 0x9e3779b9;
    ...
    for (let i = 0; i < input.length; i += 1) {
      ...
      h1 ^= code;
      // FNV-1a prime (32-bit), amplifica cambios por cada carácter.
      h1 = Math.imul(h1, 0x01000193);

      h2 ^= code + i;
      // Mezcla secundaria para evitar patrones simples.
  ...
  function deriveDigest(text, salt, rounds) {
    ...
    // Repetimos rounds para simular "work factor" educativo tipo bcrypt.
  ...
  function safeEqual(a, b) {
    ...
    // Comparación en tiempo casi constante para evitar fugas obvias por timing.
    let mismatch = 0;
